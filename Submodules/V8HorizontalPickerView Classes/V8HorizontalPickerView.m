...
@synthesize elementFont, textColor, selectedTextColor,selectedElementFont;
...
- (V8HorizontalPickerLabel *)labelForForElementAtIndex:(NSInteger)index withTitle:(NSString *)title {
	...
	elementLabel.normalFont            = self.elementFont;
    elementLabel.font =elementLabel.normalFont;
    elementLabel.selectedFont    = self.selectedElementFont;
...
- (void)scrollViewDoubleTap:(UITapGestureRecognizer *)recognizer {
	if (recognizer.state == UIGestureRecognizerStateRecognized) {
        SEL setSelectedSelector     = @selector(horizontalPickerView:didDoubleClickElementAtIndex:);
        if (self.delegate && [self.delegate respondsToSelector:setSelectedSelector]) {
...
@synthesize selectedElement, selectedStateColor, normalStateColor,selectedFont,normalFont;
