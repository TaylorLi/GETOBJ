...
- (NSString *)format:(L4LogEvent *)event
{
    ...
    @synchronized(self) {
        ...
        if(event.exception!=nil){
            [formattedString appendFormat:@"\n***Exception Info***\nName:%@\nReason:%@\nUser Info:%@",event.exception.name,event.exception.reason,event.exception.userInfo];
        }
	}
						...
						case 'd':
						{
							...
							if (([scanner scanLocation] + charsToSkip) < [[scanner string] length] && 
								...
								if ([scanner scanUpToString:@"}" intoString:&tempString2]) {
                                    ...
                                    [fmt setDateFormat:tempString2];                                    
                                    tempString = [fmt stringFromDate:[logEvent timestamp]];
