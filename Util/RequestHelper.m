...
{    
    ...
    if(byPost){
        ...
        if(async){
            return [RequestHelper getJsonInBackground:[NSString stringWithFormat:@"%@%@",url ,[self getParamsByDictionay:pars]] funCompleted:onCompleted]; 
        }
        else{
            return [RequestHelper getJsonSynchronous:[NSString stringWithFormat:@"%@%@",url ,[self getParamsByDictionay:pars]] funCompleted:onCompleted]; 
        }
    }
}
