...
{    
    ...
    if ([event respondsToSelector:@selector(_gsEvent)]) {
        ...
        if (eventMemory) {
            ...
            if(lastPressKeyAndRepeatTestTimers==nil){
                lastPressKeyAndRepeatTestTimers=[[NSMutableDictionary alloc] initWithCapacity:3];
            }
            if (eventType == GSEVENT_TYPE_KEYUP||eventType == GSEVENT_TYPE_KEYDOWN) {
                ...
                NSNumber *keyCodeNum=  [NSNumber numberWithShort:keycode[0]];
                if(eventType == GSEVENT_TYPE_KEYDOWN)
                {                    
                    NSDictionary *userInfo = [[NSDictionary alloc] initWithObjectsAndKeys:keyCodeNum, @"keycode", [NSNumber numberWithInt:eventFlags], @"eventFlags",[NSNumber numberWithBool:shift],@"shift",[NSNumber numberWithBool:command],@"command",[NSNumber numberWithBool:option],@"option",[NSNumber numberWithBool:control],@"control", nil];
                    [[NSNotificationCenter defaultCenter] postNotificationName:UIEventGSEventKeyUpNotification object:nil userInfo:userInfo];
                    if(![lastPressKeyAndRepeatTestTimers containKey:keyCodeNum]){
                        NSTimer *repeatTimer= [NSTimer scheduledTimerWithTimeInterval:TEST_KEY_REPEATE_PRESS_INTEVAL target:self selector:@selector(keyPressRepeatTest:) userInfo:userInfo repeats:YES];
                        [lastPressKeyAndRepeatTestTimers setObject:repeatTimer forKey:keyCodeNum];
                    }
                }
                else{
                    NSLog(@"Key up");
                    if([lastPressKeyAndRepeatTestTimers containKey:keyCodeNum]){
                        NSTimer *repeatTimer=   [lastPressKeyAndRepeatTestTimers objectForKey:keyCodeNum];
                        [repeatTimer invalidate];
                        [lastPressKeyAndRepeatTestTimers removeObjectForKey:keyCodeNum];
                    }
                }
                /*
                 Some Keycodes found
