...
{
    return [self queryList:@"select * from PEDSleepData where targetId=? and optDate between ? and ? order by optDate" parameters:[[NSArray alloc] initWithObjects:targetId,dateFrom,dateTo, nil]];
}
...
-(NSArray *)queryListFromDateNeedEmptySorted:(NSDate *)dateFrom toDate:(NSDate *) dateTo withTargetId:(NSString*) targetId
{
    ...
    for (NSTimeInterval i=0; i<=totalDays; i++) {
        ...
        BOOL find=NO;
        for (PEDSleepData *data in array) {
        ...
        if(!find){
            PEDSleepData *emptyData =  [[PEDSleepData alloc] init];
...
{
    return [self queryObjectBySql:@"select * from PEDSleepData where targetId = ? order by optDate desc LIMIT 1" parameters:[[NSArray alloc] initWithObjects:targetId, nil]];
}

-(PEDSleepData *)getFirstUploadData :(NSString*) targetId
{
    return [self queryObjectBySql:@"select * from PEDSleepData where targetId = ? order by optDate LIMIT 1" parameters:[[NSArray alloc] initWithObjects:targetId, nil]];
}
...
{
    PEDSleepData *data=[self getLastUploadData :targetId];
    if(data)
        return  data.optDate;
    else
        return nil;
...
{
    PEDSleepData *data = [self queryObjectBySql:@"select * from PEDSleepData where targetId = ? order by updateDate desc LIMIT 1" parameters:[[NSArray alloc] initWithObjects:targetId, nil]];
    return data?data.updateDate:nil;
}

-(NSDate *)getPreviosOptDate :(NSDate*) date withTarget:(NSString *)target
{
    PEDSleepData* data = [self queryObjectBySql:@"select * from PEDSleepData where targetId = ? and optDate < ? order by updateDate desc LIMIT 1" parameters:[[NSArray alloc] initWithObjects:target,date, nil]];
    return data?data.optDate:nil;
}

-(NSDate *)getNextOptDate :(NSDate*) date withTarget:(NSString *)target
{
    PEDSleepData* data = [self queryObjectBySql:@"select * from PEDSleepData where targetId = ? and optDate > ? order by updateDate LIMIT 1" parameters:[[NSArray alloc] initWithObjects:target,date, nil]];
    return data?data.optDate:nil;
}

-(NSDate *)getLastDateWithTarget:(NSString *)target between:(NSDate *)datefrom to:(NSDate *)dateTo
{
    PEDSleepData* data = [self queryObjectBySql:@"select * from PEDSleepData where targetId = ? and optDate < ? and optDate >= ? order by updateDate desc LIMIT 1" parameters:[[NSArray alloc] initWithObjects:target,dateTo,datefrom, nil]];
    return data?data.optDate:nil;
}
